...
import FacilityPure
extension Cocoapods {
  func deleteWrongSpecs(ctx: ContextCommon, path: Ctx.Sys.Absolute) throws {
  ...
  func installSpecs(ctx: ContextCommon, path: Ctx.Sys.Absolute) throws {
    for spec in specs {
  ...
  func resetSpecs(ctx: ContextCommon, path: Ctx.Sys.Absolute) throws {
    for spec in specs {
      let git = try Ctx.Git.make(root: .make(value: "\(path.value)/\(spec.name)"))
  ...
  mutating func updateSpecs(ctx: ContextCommon, path: Ctx.Sys.Absolute) throws {
