...
//    public internal(set) var targetTeams: Set<String> = []
...
//      diff: [String]
//    ) -> Self {
//      var result = Self(fusion: fusion, ctx: ctx, state: state)
//      result.diffTeams = ctx.ownage
//        .filter({ diff.contains(where: $0.value.isMet(_:)) })
//        .keySet
//      result.sourceTeams = ctx.rules.sourceBranch
//        .filter({ $0.value.isMet(fusion.source.name) })
//        .keySet
//      result.targetTeams = ctx.rules.targetBranch
//        .filter({ $0.value.isMet(fusion.target.name) })
//        .keySet
//      if fusion.proposition {
//        result.authorshipTeams = ctx.rules.authorship
//          .filter({ $0.value.intersection(state.authors).isEmpty.not })
//          .keySet
//        let teams = result.diffTeams
//          .union(result.sourceTeams)
//          .union(result.targetTeams)
//          .union(result.authorshipTeams)
//        result.randomTeams = ctx.rules.randoms
